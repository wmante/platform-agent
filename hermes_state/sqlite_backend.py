#!/usr/bin/env python3
"""
SQLite backend for Hermes Agent state storage.

Provides persistent session storage with FTS5 full-text search.
This is the default backend (used when HERMES_DB_URL is unset or sqlite://).
...
class SQLiteBackend(StateBackend):
    ...
    def sanitize_title(title: Optional[str]) -> Optional[str]:
        ...
        if len(cleaned) > SQLiteBackend.MAX_TITLE_LENGTH:
            raise ValueError(
                f"Title too long ({len(cleaned)} chars, max {SQLiteBackend.MAX_TITLE_LENGTH})"
            )
